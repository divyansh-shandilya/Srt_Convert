...
```bash
npm install -g Srt_Convert
```

### From source

```bash
git clone https://github.com/divyansh-shandilya/Srt_Convert.git
cd Srt_Convert
npm install
npm link
```
...
```bash
Srt_Convert [input] [output] [options]
```
...
Use Srt_Convert as a Node.js module:

```javascript
const SrtConvert = require('Srt_Convert');
...
```bash
Srt_Convert youtube-video.sbv youtube-video.srt
```

### Video Editor Compatibility
Convert to SSA format for subtitle editing software:
```bash
Srt_Convert video.srt video.ssa
```

### Web Deployment
Convert to VTT for HTML5 video players:
```bash
Srt_Convert video.srt video.vtt
```

### Timing Synchronization
If subtitles are out of sync with video:
```bash
Srt_Convert out-of-sync.srt synchronized.srt --shift -500
```
...
**Q: Does Srt_Convert support .sub files?**  
...
A: Currently, Srt_Convert is CLI-only, but a web interface is planned for future releases.
